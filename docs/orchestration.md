...
- Permissions: User `www-data` does not need to be added to the system's `docker` group - instead, a sudoers entry (provisioned by the infrastructure repo's `ansible/host-config.yml`) enables user `www-data` to run `/var/www/prod/maas-webapp/bin/docker-cli-wrapper.sh` with superuser privileges
