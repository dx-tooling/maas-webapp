...
final class Version20250813151000 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        if ($platform === 'mysql') {
            $existsRaw = $this->connection->fetchOne(
                "SELECT 1 FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'account_cores' AND INDEX_NAME = 'uniq_account_cores_email' LIMIT 1"
            );
            if ($existsRaw !== false) {
    ...
    public function down(Schema $schema): void
    {
        ...
        if ($platform === 'mysql') {
            $existsRaw = $this->connection->fetchOne(
                "SELECT 1 FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'account_cores' AND INDEX_NAME = 'UNIQ_CB89C7FEE7927C74' LIMIT 1"
            );
            if ($existsRaw !== false) {
