...
async function get(key) {
    ...
    const registryEndpoint = process.env.MAAS_MCP_INSTANCE_DATA_REGISTRY_ENDPOINT;
    const registryBearer = process.env.MAAS_MCP_INSTANCE_DATA_REGISTRY_BEARER;
    const instanceUuid = process.env.MAAS_MCP_INSTANCE_UUID;

    // Validate environment variables
    if (!registryEndpoint) {
        throw new Error("MAAS_MCP_INSTANCE_DATA_REGISTRY_ENDPOINT environment variable is not set");
    }
    if (!registryBearer) {
        throw new Error("MAAS_MCP_INSTANCE_DATA_REGISTRY_BEARER environment variable is not set");
    }
    if (!instanceUuid) {
        throw new Error("MAAS_MCP_INSTANCE_UUID environment variable is not set");
    }
